...
if (isset($_GET['upload_status'])) {
    switch ($_GET['upload_status']) {
        case 'success': $uploadMsg = 'Signed PDF uploaded successfully.'; break;
        case 'error':   $uploadMsg = 'Error processing signed PDF.'; break;
        case 'deleted': $uploadMsg = 'Signed PDF deleted.'; break;
    }
}
if (isset($invId)) {
...
<div id="app" v-if="invoice">
      ...
      <div class="row">
        <div class="col-md-6">
          <div class="mb-3"><label class="form-label">Invoice Number</label>
            <input v-model="invoice.invoice_number" class="form-control" />
          </div>
          <div class="mb-3"><label class="form-label">Date</label>
            <input type="date" v-model="invoice.date" class="form-control" />
          </div>
          <div class="mb-3">
  ...
  <div class="card mb-4">
    ...
    <div class="card-body">
      ...
      <p>
        Signed PDF:
        <a href="/view_signed_pdf.php?file=<?= urlencode($signedFile) ?>" target="_blank"><?= htmlspecialchars($signedFile) ?></a>
        <form method="POST" action="delete_signed_pdf.php" class="d-inline ms-3">
          ...
          <button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('Delete signed PDF?');">Delete</button>
        </form>
      </p>
...
createApp({
  ...
  methods: {
    ...
    calculateTotals() {
      ...
      this.totals = { subtotal: sub, tax: tax, total: sub + tax };
    },
    formatCurrency(v, c) {
    ...
    loadCompany() {
      if (this.selectedCompany) {
        this.myCompany = { ...this.selectedCompany };
